...
class Op(object):
    ...
    def infer_shape(self, node, input_aakar):
...
class AddOp(Op):
    ...
    def infer_shape(self, node, input_aakar):
        ...
        if input_aakar[0][0] or input_aakar[0][1]:
            return input_aakar[1]
        else:
            return input_aakar[0]
...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_aakar):
        ...
        r = input_aakar[0][0] if not node.matmul_attr_trans_A else input_aakar[0][1]
        c = input_aakar[1][1] if not node.matmul_attr_trans_B else input_aakar[1][0]
        return (r,c)
...
class ReduceSumAxisZeroOp(Op):
    ...
    def infer_shape(self, node, input_aakar):
        ...
        shape = (1,)
        # print(input_aakar)
        for i in range(len(input_aakar[0])-1):
            shape += (input_aakar[0][i+1],)
        ...
        for node in self.topo_order:
            if node not in self.node_to_shape_map:
                ...
                for input_node in node.inputs:
                    node_input_aakar.append(self.node_to_shape_map[input_node])
                self.node_to_shape_map[node] = node.op.infer_shape(node, node_input_aakar)
